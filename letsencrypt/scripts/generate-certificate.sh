...
LEGO_BIN=/home/victor/wordpress-5.2.3-0/letsencrypt/lego
CERTIFICATES_DIR=/home/victor/wordpress-5.2.3-0/letsencrypt/certificates
...
is_apache_installed() {
    if [ -d "/home/victor/wordpress-5.2.3-0/apache2" ]; then
        true
    else
        false
    fi
...
stop_server() {
    /home/victor/wordpress-5.2.3-0/ctlscript.sh stop "$(server_type)"
}

start_server() {
    /home/victor/wordpress-5.2.3-0/ctlscript.sh start "$(server_type)"
}

restart_server() {
    /home/victor/wordpress-5.2.3-0/ctlscript.sh restart "$(server_type)"
}

is_varnish_enabled() {
    if [[ -f "/home/victor/wordpress-5.2.3-0/varnish/scripts/ctl.sh" ]]; then
        true
    else
        false
    fi
...
is_varnish_running() {
    if is_varnish_enabled && "/home/victor/wordpress-5.2.3-0/ctlscript.sh" status varnish 2> /dev/null | grep "already running" > /dev/null 2>&1 ; then
        true
    else
        false
    fi
...
stop_varnish() {
    "/home/victor/wordpress-5.2.3-0/ctlscript.sh" stop "varnish"
}

start_varnish() {
    "/home/victor/wordpress-5.2.3-0/ctlscript.sh" start "varnish"
}
...
web_server_conf_dir() {
    if is_apache_installed; then
        echo "/home/victor/wordpress-5.2.3-0/apache2/conf"
    else
        echo "/home/victor/wordpress-5.2.3-0/nginx/conf"
    fi
}
...
modify_configuration() {
    if is_apache_installed; then
        sed -i "s;\s*SSLCertificateFile\s.*;  SSLCertificateFile \"/home/victor/wordpress-5.2.3-0/apache2/conf/${domains[0]}.crt\";g" "$(web_server_conf_dir)/bitnami/bitnami.conf"
        sed -i "s;\s*SSLCertificateKeyFile\s.*;  SSLCertificateKeyFile \"/home/victor/wordpress-5.2.3-0/apache2/conf/${domains[0]}.key\";g" "$(web_server_conf_dir)/bitnami/bitnami.conf"
    else
...
EOF
    ...
    if is_apache_installed; then
        server_restart_command="/home/victor/wordpress-5.2.3-0/apache2/bin/httpd -f /home/victor/wordpress-5.2.3-0/apache2/conf/httpd.conf -k graceful"
    else
        server_restart_command="/home/victor/wordpress-5.2.3-0/nginx/sbin/nginx -s reload"
    fi

    crontab $USER -l 2> /dev/null | {
        cat
        echo "0 0 1 * * $SUDO $LEGO_BIN --path=\"/home/victor/wordpress-5.2.3-0/letsencrypt\" --tls --email=\"${email}\" ${domain_args} renew && $SUDO $server_restart_command"
...
EOF
    ...
    "$LEGO_BIN" --path "/home/victor/wordpress-5.2.3-0/letsencrypt" --tls --email="${email}" ${domain_args} run
